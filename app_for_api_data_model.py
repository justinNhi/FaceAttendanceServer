...
def get_person():
    session_sql = SessionSql()
    array_return = []
    rs = session_sql.query(Person).filter(Person.USED_STATUS != 0).order_by(desc(Person.PERSON_ID)).all()
    try:
        if len(rs) > 0:
            for row in rs:
                json_return = {}
                ...
                if getattr(row, "PERSON_MIDDLE_NAME") is not None:
...
def test_insert_check_in_check_out_handle():
    session_sql = SessionSql()
    month = 7
    year = 2022
    start_date = datetime.strptime('26/6/2022', '%d/%m/%Y')
    end_date = datetime.strptime('26/7/2022', '%d/%m/%Y')
...
#     test_insert_check_in_check_out_handle()
