...
from app_for_api_data_model import get_check_all, data_person_label_embedding, insert_auto_attendance, insert_person, \
    get_person, data_attendances_date_month_year, person_attendance_table
    ...
    try:
        ...
        if len(rects) > 0:
            for rect in rects:
                ...
                retval, buffer = cv2.imencode('.jpg', faceAligned)
                face_img_base64 = base64.b64encode(buffer)
                person_id_number, name, probability, id_person = predict_guest(
                    faceAligned)  # get prediction return label and probability
                if id_person == 0:
                    error_code = 404
                    msg = 'Not Found'
                    get_reload = 0
...
#
#             type_of_history = json_data['type_of_history']
#             # data_type_of_history = json_data['data_type_of_history']
#
#             json_person_account = check_log_in(account_name, account_password)
#
#             date_today = datetime.today()
#             if int(json_person_account['ACCOUNT_TYPE_ID']) == 2:
#                 if int(type_of_history) == 1:
#                     # json_to_method = {
#                     #     'TODAY': date_today,
#                     # }
#                     json_send = get_data_history(date_today)
#                 elif int(type_of_history) == 2:
#                     # json_to_method = {
#                     #     'MONTH': date_today.month,
#                     #     'YEAR': date_today.year,
#                     #
#                     # }
#                     json_send = get_data_history(date_today.month, date_today.year)
#                 else:
#                     # json_to_method = {
#                     #     'YEAR': date_today.year,
#                     #
#                     # }
#                     json_send = get_data_history(date_today.year)
#         except Exception as e:
#             exc_type, exc_obj, exc_tb = sys.exc_info()
#             fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
#             print(e, exc_type, fname, exc_tb.tb_lineno)
